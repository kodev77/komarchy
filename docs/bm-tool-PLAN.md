...
**Follow chromium's focused tab.** The user can switch tabs inside chromium (click a tab, Ctrl+Tab, close the active tab so chromium promotes the next one) without bm observing the event. `_refresh_live` closes that gap by querying the **W3C Page Visibility API** on each candidate: `cdp.visible_tab(raw_tabs, prefer_id=self._active_tab_id)` opens a one-shot WebSocket to a tab's debugger endpoint, calls `Runtime.evaluate({"expression": "document.visibilityState"})`, and returns the tab whose state is `'visible'`. The `prefer_id` argument is probed first so the steady-state cost is one WS round-trip (the user isn't usually switching tabs every refresh tick); falls through to scanning `raw_tabs` in order on a miss. Each refresh tick (every 300 ms) compares the resolved visible tab's id to `self._active_tab_id`; on divergence, bm updates both `_active_url` and `_active_tab_id` to match, **and the bm cursor moves with the highlight** — `_restore_cursor` is scheduled (via `call_after_refresh`, after `_rebuild_tree`'s own prev-URL restore so the active-follow move wins) onto the new active row. The active-follow path also parks the cursor (`tree.cursor_active = False`) for the duration of the rebuild and re-enables it inside the follow callback, so the brief line-0 stop after `tree.clear()` doesn't render the hover-dim on Workspace as a flash; the prior parked-state value is captured and restored, preserving an Esc-park the user had set. `active_changed` only fires for chromium-driven shifts (bm's own activations preset `_active_tab_id`), so normal j/k navigation in the sidebar isn't yanked. Returns `None` when no tab reports visible (chromium minimized/occluded, or every WS probe failed) — the prior active highlight stays in place rather than blanking.

**Why visibility, not `/json/list[0]` MRU.** Earlier the active-tab follower picked `raw_tabs[0]` and trusted CDP's `/json/list` to be MRU-ordered. That heuristic broke on **background-opened tabs** (right-click → "Open link in new tab", middle-click): chromium pushes a freshly-created tab to MRU position 0 *regardless* of whether it was opened in the foreground, and leaves it there indefinitely until the user manually clicks it. bm would see the new tab at index 0 next tick and conclude "user switched to this tab," driving `_active_tab_id`, the highlight, the cursor follow, and even the cross-workspace flip onto a tab the user hadn't actually switched to. A previous brand-new-tab-id sticky heuristic only papered over the first tick; chromium's MRU never bubbles the focused tab back to position 0 without an explicit click, so the wrong tab kept winning on subsequent ticks. The visibility API is the spec-correct signal: exactly one tab per chromium window reports `visible` at any time, and a background-opened tab reports `hidden` even though it sits at MRU 0. Empirically verified against a live chromium with a background-opened 37signals tab at `/json/list[0]` and the actually-focused omarchy.org at `/json/list[1]` — `document.visibilityState` returned `'visible'` for omarchy.org and `'hidden'` for everything else including 37signals, while raw MRU order shuffled across queries. Cost: one WS handshake per refresh tick in steady state (~5 ms localhost), bounded scan when the visible tab moves. Adds `websockets >= 13` (sync client) as a dep.
...
| `h` / `l` (or `←` / `→`, or `Shift+h` / `Shift+l` while searching) | Cycle a group header through three states (collapsed → filtered → expanded). `h` shrinks one step (expanded → filtered → collapsed; terminal stays at collapsed and falls through to the existing parent-move semantic for non-group rows); `l` grows one step (collapsed → filtered → expanded; terminal stays at expanded). The middle "filtered" state keeps the header expanded but only renders saved children currently paired with an open chromium tab — see "Three-state group folder" below. State is persisted per-workspace in `state.json` so collapses and filter selections survive workspace switches and bm restarts. |
...
| `y` | **Yank live URL** to the system clipboard via `wl-copy` (Wayland-only — bm targets Hyprland, so X11 fallbacks aren't worth the branch). Prefers the *live* chromium URL (looked up by `row.tab_id` in `self._live`) over the row's stored url, so a saved row whose tab has navigated away from the saved URL yanks what's actually in the address bar. Unpaired saved rows fall back to `row.url` (the saved URL) since that's the only URL we have for them. Status `Copied URL` on success, `Copy failed (wl-copy missing?)` otherwise. The clipboard call passes `stdout=DEVNULL, stderr=DEVNULL` rather than `capture_output=True` because wl-copy daemonizes a child to keep the Wayland selection alive — the daemon inherits the parent's stdout/stderr pipes, so capture_output blocks `subprocess.run` waiting for EOF that never comes (manifests as a 2s `TimeoutExpired` and a misleading "wl-copy missing" status). |
| `Y` | **Yank saved URL** to the system clipboard. Saved-only — copies `row.url` (the canonical saved URL, regardless of where the paired tab has navigated). Loose live rows are rejected with status `Not a saved tab`; non-saved rows (Workspace title, group header, spacer) no-op. Pair to `y`: lowercase yanks the live address-bar URL, uppercase yanks what was bookmarked. |
| `m` | **MOVE / reorder mode** — modal toggle for in-place reordering. Press `m` on a movable row → enters MOVE mode (status bar suffix shows `[move]`); `j`/`k` swap the carried row/group with its in-scope neighbor; `m`/`Esc`/`Enter` exits. Scope is determined per row kind: saved tab in a group ↔ siblings in same group, Essentials tab ↔ other Essentials, group header ↔ other group headers (carries children with it), loose Open Tab ↔ other loose Open Tabs in same workspace. Workspace title / spacer / divider rows reject the keybind with status `Can't move this row`. While `_in_move_mode` is set, `_in_modal_state()` returns true, so all non-movement actions (o/t/s/d/y/Y/h/l/Enter/etc.) silently bail; `j`/`k` explicitly bypass that gate to drive the swap. The `[move]` suffix marker rides in the same right-aligned slot as `[rename]`/`[preview]`. See "MOVE / reorder mode" below for the persistence story per row kind. |
...
| `Esc` | Contextual dismiss, six tiers: (0) **in MOVE mode**, exits MOVE; (1) in any edit/picker mode, cancels via `_cancel_edit`; (2) in search, clears the filter; (3) in help, closes help; (4) with the cursor visible, parks the cursor (see "Esc-to-park cursor" above); (5) once parked, closes chromium and exits the TUI. |
...
The `HELP_LINES` table that drives the rendered list mirrors the Keybindings section above — including the yank pair (`y` / `Y`), `m` move/reorder, and the updated `h/l` description (`collapse / filter / expand`). Workspace controls (`w` switch workspace, `W` new workspace, `;` next workspace) are listed alongside `r` rename and the rest of the saved-tab actions.
...
Two clipboard keybinds, one underlying helper. Both call `actions.copy_to_clipboard(text)` which pipes `text` to `wl-copy` (Wayland-only — bm targets Hyprland). The helper passes `stdout=DEVNULL, stderr=DEVNULL` rather than `capture_output=True`: wl-copy daemonizes a child to keep the Wayland selection alive across its parent's exit, and the daemon inherits the parent's stdout/stderr pipes. With pipe-backed `capture_output`, `subprocess.run` waits for EOF on the captured streams that never comes (the daemon holds the pipes open indefinitely), trips the 2 s timeout, and raises `TimeoutExpired` — manifesting as a misleading `Copy failed (wl-copy missing?)` status even though wl-copy worked. DEVNULL gives the daemon closed-equivalent FDs; subprocess.run returns immediately once the parent forks the daemon and exits.

Behavior split:

- **`y` — yank live URL.** Looks up `row.tab_id` in `self._live`; if found, copies the live tab's current URL (so a saved row whose paired chromium tab has navigated away from the saved URL yanks what's actually in the address bar). Falls back to `row.url` for unpaired saved rows since that's the only URL available. Status `Copied URL`.
- **`Y` — yank saved URL.** Saved-only — copies `row.url` (the canonical entry from `saved-tabs.json`). Loose live rows reject with status `Not a saved tab`; non-tab rows (Workspace title, group header, spacer) silently no-op. Status `Copied saved URL`.

## MOVE / reorder mode (`m`)

Modal toggle for in-place reordering. Press `m` on a movable row → `_in_move_mode = True`; the right-aligned status-bar slot displays `[move]` (same dim tier as `[rename]`/`[preview]`). While in mode, `j`/`k` swap the carried row with its in-scope neighbor. `m`, `Esc`, or `Enter` exits.

**Scope per row kind:**

| Cursor on | Swaps with | Persistence |
|---|---|---|
| Saved tab in a group | Adjacent saved sibling in same `(group, workspace)` | `_saved` list is reordered in place; `store.save_all(self._saved)` writes `saved-tabs.json` (file order is render order) |
| Essentials tab | Adjacent Essentials entry (workspace ignored — Essentials is global) | Same `_saved` reorder + write path |
| Group header | Adjacent group header in current workspace; carries its children with it | `groupOrderByWorkspace[workspace_id]` is rewritten in `state.json` via `store.set_group_order` (replaces alphabetical; new groups appended in creation order — see "Group order" below) |
| Loose Open Tab | Adjacent loose live tab in same workspace, excluding tabs paired to a saved row (those render under their saved header, not in Open Tabs) | Mutates `self._live_order` in place + immediately re-sorts `self._live` via `_stable_sort_live(self._live)` so `_rebuild_tree` reads consistent state on the next paint. **Not persisted** — chromium tab ids reset across restarts |
| Workspace title / spacer / divider / pending-new-group preview placeholder | Rejected | Status `Can't move this row`, no mode change |

**Modal-state plumbing.** `_in_move_mode` is added to `_in_modal_state()` so all non-movement actions (o/t/s/d/y/Y/h/l/Enter/etc.) silently bail. `j`/`k` (and `J`/`K`) explicitly bypass that gate by checking `_in_move_mode` first and dispatching to `_move_carried(direction)` instead of `tree.action_cursor_*`. `m` itself also checks `_in_move_mode` before the modal guard, so re-pressing toggles out instead of being swallowed by `_in_modal_state()`. `action_quit_to_browser` adds a tier-0 branch that exits MOVE first (ahead of the existing edit/picker dismiss tiers).

**`auto_expand` suppression on Enter.** Textual's `Tree` widget has a built-in `@on(NodeSelected)` listener that calls `node.toggle()` on branch nodes when `auto_expand=True` (the default). It runs before our App-level `on_tree_node_selected`, so without intervention an Enter on a group header in MOVE mode would collapse/expand the group AND post `NodeCollapsed`/`NodeExpanded` (which our persistence handler would write to disk) before our handler got a chance to translate Enter into "exit move mode." `action_enter_move_mode` flips `tree.auto_expand = False` while in mode; `_exit_move_mode` restores it. Net effect: Enter on a group cleanly exits MOVE without an unintended toggle.

**Live-tab move desync fix.** On the loose-live path, mutating `_live_order` alone is insufficient — `_rebuild_tree` reads `self._live` (already-sorted), and `_refresh_live`'s `_tabs_differ` gate compares `(id, url, title)` as sets, so a pure reorder doesn't trigger a refresh-driven rebuild. Without explicitly re-sorting `self._live` after the swap, `_rebuild_tree` would render stale order until something else (tab open/close, title change) forced a rebuild — and a follow-up `j`/`k` would compute its position from the new `_live_order` against the stale tree, producing "move only works once" behavior. The fix is one line: `self._live = self._stable_sort_live(self._live)` right after mutating `_live_order`.

**Auto-exit on focus loss.** `on_app_blur` (Textual's `AppBlur` event handler) calls `_exit_move_mode` whenever bm loses window focus. Closes a UX hole: external signal handlers (Super+Alt+J/K/;) gate on `_in_move_mode` and silently no-op while it's set, so leaving bm in MOVE mode would make those keybinds appear broken until the user came back and Esc'd. Auto-exit on blur ensures the next external keybind works normally regardless of how the user left bm.

**Signal-handler gates.** `_cycle_step` (SIGUSR1/SIGUSR2 → tab next/prev) and `_cycle_workspace_step` (SIGRTMIN → workspace next) both `return` early when `_in_move_mode` is True. Catches the corner case where focus is somehow still on bm when an external signal arrives — the user cancels MOVE manually rather than getting yanked off the carried row.

**Initial-status hint suppression.** Earlier the mode entry posted a 3 s status `MOVE: j/k reorder, esc to exit`; that was removed in favor of the persistent `[move]` suffix marker, which sits quietly at the right edge for the entire mode lifetime and costs zero focal attention.

## Group order (per-workspace, creation-order)

Saved-group headers no longer render alphabetically. Each workspace stores an explicit order list at `state.groupOrderByWorkspace[workspace_id]`; `_rebuild_tree` calls `_ordered_group_names(groups)` instead of `sorted(groups)`. The resolver:

1. Walks the persisted order list, keeping only names that exist in the current `groups` dict (renamed/deleted groups self-prune).
2. Appends any group not yet in the persisted list, in iteration order of `self._saved` filtered to the current workspace — and since `add_saved` appends to `_saved`, this is **creation order**.

Net effect: the first time the user has groups in a workspace, they render in the order they were created. The first MOVE-mode swap of a group header writes the full effective order back to `state.json`, ratcheting all currently-visible groups into the persisted list (so subsequent renders use the persisted order without falling through to creation-order discovery for the rest of the session). Renamed groups revert to the end of the order on the first rebuild after the rename (the stale name filters out, the new name gets appended via creation-order discovery); deleted groups silently filter out and stay in the persisted list as harmless residue until the user reorders again.

## Three-state group folder (expanded / filtered / collapsed)

Each saved-group header lives in one of three states. The state cycles via `h`/`l` on the header; spacebar/click toggles still do binary expand/collapse and clear any prior filtered state.

| State | What it shows | Persisted in |
|---|---|---|
| Expanded | All saved children in the group | (default — group not in either set) |
| Filtered | Header expanded but only saved children currently paired with an open chromium tab; unopened saved entries are hidden | `state.filteredGroupsByWorkspace[workspace_id]` (list of group names) |
| Collapsed | Children hidden | `state.collapsedGroupsByWorkspace[workspace_id]` (list of group names) |

The two persisted sets are **mutually exclusive** in normal use — `_cycle_group_state` discards the group from both before adding it to the new one's set. A binary spacebar/click toggle (handled by `_record_group_collapse` via `on_tree_node_expanded`/`on_tree_node_collapsed`) also strips filtered membership so explicit binary intent wins cleanly.

**`h`/`l` are the cycle drivers.** `action_collapse` and `action_expand` route group nodes through `_cycle_group_state(node, direction)` which directly mutates the persisted sets and calls `_rebuild_tree` — bypassing Textual's `node.expand()`/`collapse()` so the on-tree-node-* handlers don't double-fire. Non-group cases keep the old behavior (`h` falls through to "move cursor up to parent"; `l` no-ops).

**Filter visual cue: tinted folder icon, no glyph swap.** The filtered group keeps the regular `_FOLDER_OPEN` glyph (tested earlier with a separate `_FOLDER_FILTERED` glyph from the MDI family — the cell-width mismatch against the existing FA folder glyphs read as visually misaligned). Differentiation is color-only: the icon picks `FolderTree._filter_icon_color(colors)` instead of accent. The text portion of the group header always stays accent — `render_label` builds the label fresh via `Text().append(...)` per segment with explicit styles, because Rich's `Text.__add__` inherits the first operand's default style and was bleeding the icon's filter color onto the group text otherwise.

**Filter color is theme-aware with an Osaka Jade override.**

- **Osaka Jade** specifically returns the hardcoded `#86EFAC` (Tailwind green-300, vivid mint). Hand-tuned because Osaka Jade's accent and `color4` are both `#509475` (greenish), so any palette-driven candidate from `color2`/`color4`/`color10` blended into the accent at small render sizes.
- **Every other theme** falls through to a "lifted accent": parse `accent` (or `color4`) via `Color.parse`, convert to HLS via `colorsys`, push lightness into `[0.70, 0.78]` and floor saturation at `0.60`, return the resulting hex. Stays in the same hue family as the rest of the theme so the cue reads as palette-coherent, but is bright enough to pop. Sample outputs: Catppuccin Mocha mauve `#cba6f7` → `#C398F6`; Gruvbox amber `#d79921` → `#F0D49E`; Tokyo Night blue `#7aa2f7` → `#95B5F9`; Nord frost `#88c0d0` → `#A5DAE9`; Rosé Pine rose `#ebbcba` → `#E9A8A5`.
- Pure brightness blends (`Color.lighten(amount)` / mix-toward-white) wash out saturation and produced too-subtle results in tests; bumping S directly keeps the lifted color vivid. Final fallback (any parse failure) is the Osaka Jade hardcoded `#86EFAC`.

The current omarchy theme name is cached on `BmApp._omarchy_theme_name` at startup (read once via `bm_theme.load_name()`) so the resolver doesn't re-read the marker file every render frame.

**Filter cue only renders when *visibly* filtering.** The icon-color override gates on `node.is_expanded and self._is_group_filtered(node) and len(node.children) > 0`. Three reasons to fall back to accent:

1. **Closed folder.** A collapsed folder can't visually express "I'm showing only the open children" — the children aren't visible to filter. Edge case: stale `state.json` where a group somehow ended up in both `collapsed` and `filtered` sets renders with the closed glyph + accent (mutually-exclusive state-machine intent wins over residual data).
2. **Open + zero paired children.** A filtered group with no chromium-paired tabs renders an open folder with an empty child list — visually identical to a normal empty/closed folder, so the brighter tint adds confusion rather than information. Falls back to accent.
3. **Not in the filtered set at all.** Trivially the default.

`FolderTree._filtered_groups: set` is cached on the app and refreshed at the top of every `_rebuild_tree`, so the membership check is O(1) per render.

## Per-workspace collapsed-group memory

Group expand/collapse state is persisted per-workspace in `state.collapsedGroupsByWorkspace`. On every `_rebuild_tree`, the resolver reads the workspace's collapsed set and passes `expand=group_name not in collapsed_groups` when adding each group node. Toggles flow through `on_tree_node_expanded`/`on_tree_node_collapsed` event handlers (`_record_group_collapse`) which:

- Skip non-main-tree events (the search overlay tree posts the same messages).
- Skip non-`_GroupMarker` nodes (live-tab leaves don't expand; the Workspace title is a leaf; the pending new-group preview header has an empty key in `_saved_nodes`).
- Reverse-look-up the group name via `_saved_nodes` (insertion order = render order from the most recent rebuild).
- Update the persisted set and **also strip filtered membership** — explicit binary toggle (spacebar/click) clears any prior filtered intent so the state can't get stuck in both sets. (h/l cycling bypasses Textual's `node.expand()`/`collapse()` and drives the persisted sets directly via `_cycle_group_state`, so this binary handler only fires on spacebar / click / explicit `node.expand()` calls.)

Renamed groups invalidate their entry (the new name isn't in the persisted set, so the group reverts to expanded; the user re-collapses, and the new name lands in the set on the next toggle). Deleted groups stay in the set as harmless residue.
...
| `~/.config/bm/state.json` | No | Local UI state. Active keys: `currentWorkspace` (uuid hex pointer into `workspaces[]` — last-viewed workspace, restored on bm boot); `openTabUrlsByWorkspace` (`{workspace_id: [url, ...]}` — per-workspace URL list rebuilt every refresh tick from the in-memory `_tab_workspace` tag map; survives chromium session-restore so workspace tags re-attach by URL match); `collapsedGroupsByWorkspace` (`{workspace_id: [group_name, ...]}` — group headers the user has collapsed, restored on workspace switch and bm boot); `filteredGroupsByWorkspace` (`{workspace_id: [group_name, ...]}` — group headers the user has set to the third "filtered" state in the h/l cycle); `groupOrderByWorkspace` (`{workspace_id: [group_name, ...]}` — user-chosen render order for saved-group headers within a workspace; new groups not yet in the list render in creation order — `add_saved` appends, so iteration order of `_saved` is creation order — and only mutated when the user actually reorders via MOVE mode). Empty workspace entries are pruned from each map so the file stays tidy. Disposable. |
...
- **Phase 1.6** (in flight, ships inside group `019`) — **W3C Page Visibility-based active-tab follower**: replaces the brittle `/json/list[0]` MRU heuristic (which spuriously promoted background-opened tabs — right-click "Open link in new tab" / middle-click — to active because chromium pushes new tabs to MRU 0 regardless of focus). `cdp.visible_tab(tabs, prefer_id=...)` opens a one-shot WebSocket per candidate, calls `Runtime.evaluate("document.visibilityState")`, returns the `'visible'` tab; `prefer_id` is probed first so steady-state cost is one WS round-trip (~5ms localhost). Adds `websockets >= 13` (sync client) as a dep. **`y` / `Y` yank URL keybinds** — `y` copies the live address-bar URL via `wl-copy`, `Y` copies the canonical saved URL; the helper passes `stdout=DEVNULL, stderr=DEVNULL` to dodge wl-copy's daemon-keeps-pipe-open hang under `capture_output=True`. **Per-workspace collapsed-group memory** — `state.collapsedGroupsByWorkspace` persists which group headers the user has collapsed, restored on workspace switch and bm boot; `on_tree_node_expanded`/`on_tree_node_collapsed` handlers (`_record_group_collapse`) keep the set in sync with binary spacebar/click toggles. **`m` MOVE / reorder mode** — modal toggle (`[move]` suffix marker) where `j`/`k` swap the carried row with its in-scope neighbor: saved tab in a group, Essentials tab, group header (carries children), or loose Open Tab. Saved-row swaps reorder `_saved` and persist via `store.save_all`; group-header swaps write `state.groupOrderByWorkspace` (replaces the alphabetical sort with creation-order for new groups, persisted-order for moved groups); live-tab swaps mutate `_live_order` + re-sort `self._live` in lockstep (the lockstep re-sort fixes a "move only works once" bug where `_rebuild_tree`'s stale `self._live` desynced from the freshly-mutated `_live_order`). Modal plumbing: `_in_move_mode` joins `_in_modal_state()` so non-movement actions bail; `j`/`k` bypass the gate; `auto_expand` is flipped off for the duration so Enter exits cleanly without toggling the group; `on_app_blur` auto-exits MOVE on focus loss; SIGUSR1/SIGUSR2/SIGRTMIN handlers gate on `_in_move_mode` and silently no-op so external keybinds don't yank the carried row mid-reorder. **Group order is creation-order, not alphabetical** — `_ordered_group_names(groups)` walks the persisted `groupOrderByWorkspace` first, then appends groups not yet in the list in `_saved`-iteration order (which is creation order since `add_saved` appends). **Three-state group folder cycle** — `h`/`l` on a group header cycles `expanded → filtered → collapsed`; the middle "filtered" state keeps the header expanded but renders only saved children currently paired with an open chromium tab. State persists in `state.filteredGroupsByWorkspace`. Folder glyph stays `_FOLDER_OPEN` for filtered (vs. trying a separate MDI glyph that came out at the wrong cell width); differentiation is a tinted icon color — `FolderTree._filter_icon_color(colors)` returns a hardcoded `#86EFAC` (vivid mint) for Osaka Jade and a "lifted accent" (HSL push of lightness to `[0.70, 0.78]`, saturation floor `0.60`) for every other theme so the cue stays palette-coherent. Cue only renders when the filter is *visibly* doing something (`node.is_expanded and is_filtered and len(node.children) > 0`); empty filtered groups and stale corrupted-state cases fall back to the regular accent. Pure code update — `019-00110-bms-app.sh`'s existing `uv tool install --force` reinstalls the package on every re-migrate; new `state.json` keys (`collapsedGroupsByWorkspace`, `filteredGroupsByWorkspace`, `groupOrderByWorkspace`) are wiped by `019-00111` rollback alongside the existing keys.
- **Phase 2** — polish: Kitty-graphics favicons (background fetch + cache already wired in `bm.favicon`; render path and multi-cell Tree variant still to build), compact-mode toggle layered on top of the graphics path (see "Compact mode" under Favicons), bulk import/export, search-across-tab-content via CDP `Runtime.evaluate`, per-group colors.
